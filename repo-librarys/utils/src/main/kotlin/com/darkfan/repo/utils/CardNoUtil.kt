...
import com.darkfan.repo.utils.ext.isBlank
import com.darkfan.repo.utils.ext.size

/**
 * @author Fanfan Gu <a href="mailto:stefan.gufan@gmail.com">Contact me.</a>
 * @date 02/05/2022 16:44
 * @desc 银行卡号相关工具类.
 */
object CardNoUtil {
     ...
     * BIN号
     */
    private val cardBin = arrayOf("621098", "622150", "622151",
     ...
     * 发卡行.卡种名称
     */
    private val cardName = arrayOf("邮储银行·绿卡通", "邮储银行·绿卡银联标准卡",
     ...
     * 检查银行卡号是否有效.
     */
    fun validCardNo(cardNo: String?): Boolean {
        if (cardNo.isBlank()) return false

        val bit = getCardCheckCode(cardNo!!.substring(0, cardNo.size() - 1))
        return if (bit == 'N') {
            false
        } else {
            return cardNo.toCharArray()[cardNo.size() - 1] == bit
        }
    }

    private fun getCardCheckCode(nonCheckCodeCardId: String): Char {
        if (nonCheckCodeCardId.isBlank() || nonCheckCodeCardId.matches("\\d+".toRegex())) {
            return 'N'
        }

        val charArray = nonCheckCodeCardId.toCharArray()
        var luhmSum = 0

        var i: Int = charArray.size - 1
        var j = 0
        while (i >= 0) {
            var k: Int = charArray[i] - '0'
            if (j % 2 == 0) {
                k *= 2
                k = k / 10 + k % 10
            }
     ...
     * 根据卡号获取银行卡开户行
     */
    fun getBankName(cardNo: String?): String {
        if (cardNo.size() < 6 || cardNo.isBlank()) {
            return ""
        }

        val bin = cardNo!!.substring(0, 6)
        var binIndex = -1
        cardBin.forEachIndexed { index, s ->
            if (bin == s) {
                binIndex = index
            }
        }

        return if (binIndex == -1) {
            ""
        } else {
            cardName[binIndex].split("·")[0]
        }
    }
     ...
     * 根据卡号获取银行卡开户行和卡种.
     */
    fun getBankNameAndType(cardNo: String?): String {
        if (cardNo.size() < 6 || cardNo.isBlank()) {
            return ""
        }
